    ...
    else
    {
      ...
      if (this.statement.connection.useFetchSizeWithLongColumn)
      {
        ...
        while (k != -1)
        {
          ...
          k = readStreamFromWire(this.data[this.lastRowProcessed], this.nbBytesRead[this.lastRowProcessed], 255, this.escapeSequenceArr, this.readHeaderArr, this.readAsNonStreamArr, this.mare, ((T4CConnection)this.statement.connection).oer);

          if (k == -1)
            continue;
          this.nbBytesRead[this.lastRowProcessed] += k;
        }
