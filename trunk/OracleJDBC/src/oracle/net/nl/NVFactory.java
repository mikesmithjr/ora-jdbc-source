...
public class NVFactory {
    public NVPair createNVPair(String paramString) throws InvalidSyntaxException, NLException {
        NVTokens localNVTokens = new NVTokens();
        localNVTokens.parseTokens(paramString);
        return _readTopLevelNVPair(localNVTokens);
    }

    private NVPair _readTopLevelNVPair(NVTokens paramNVTokens) throws InvalidSyntaxException,
            NLException {
    ...
    private NVPair _readNVPair(NVTokens paramNVTokens) throws InvalidSyntaxException, NLException {
        int i = paramNVTokens.getToken();
        paramNVTokens.eatToken();
        if ((i != 1) && (i != 3)) {
    ...
    private NVPair _readRightHandSide(NVPair paramNVPair, NVTokens paramNVTokens)
        ...
        if (i == 2) {
            ...
            throw new InvalidSyntaxException("UnexpectedChar-04605", localObject);
        }
        return (NVPair) paramNVPair;
    }

    private String _readNVLiteral(NVTokens paramNVTokens) throws InvalidSyntaxException,
            NLException {
        int i = paramNVTokens.getToken();
        if (i != 8) {
    ...
    private String getContext(NVTokens paramNVTokens) throws NLException {
        return " " + paramNVTokens.popLiteral() + " " + paramNVTokens.popLiteral() + " "
                + paramNVTokens.popLiteral();
    }
}

/*
 * Location: D:\oracle\product\10.2.0\client_1\jdbc\lib\ojdbc14_g.jar Qualified Name:
 * oracle.net.nl.NVFactory JD-Core Version: 0.6.0
 */