...
public class NavDescription extends Description implements NavSchemaObject {
    ...
    public void navigate(ConnStrategy paramConnStrategy, StringBuffer paramStringBuffer) {
        ...
        if (this.sourceRoute) {
            if (!backwardCompatibilityCase(this.children, paramConnStrategy)) {
                ...
                ((NavSchemaObject) this.activeChildren.elementAt(0)).navigate(paramConnStrategy,
                                                                              paramStringBuffer);
                for (i = 1; i < this.activeChildren.size(); i++)
                    ((NavSchemaObject) this.activeChildren.elementAt(i))
                            .addToString(paramConnStrategy);
            }
            setConnectionOption(paramConnStrategy, paramStringBuffer);
            closeNVPair(paramConnStrategy);
        } else {
            this.activeChildren = NavDescriptionList.setActiveChildren(this.children,
                                                                       this.failover,
                                                                       this.loadBalance);
            for (i = 0; i < this.activeChildren.size(); i++)
                ((NavSchemaObject) this.activeChildren.elementAt(i)).navigate(paramConnStrategy,
                                                                              paramStringBuffer);
            closeNVPair(paramConnStrategy);
        }
        this.done = true;
    ...
    public void closeNVPair(ConnStrategy paramConnStrategy) {
        for (int i = paramConnStrategy.cOpts.size() - 1; (i >= 0)
                && (!((ConnOption) paramConnStrategy.cOpts.elementAt(i)).done); i--) {
    ...
    private boolean backwardCompatibilityCase(Vector paramVector, ConnStrategy paramConnStrategy) {
        if ((paramVector.size() == 1) && (((NavSchemaObject) paramVector.elementAt(0)).isA() == 1)
                && (!((NavAddressList) paramVector.elementAt(0)).sourceRoute)) {
 ...
 * oracle.net.resolver.NavDescription JD-Core Version: 0.6.0
 */