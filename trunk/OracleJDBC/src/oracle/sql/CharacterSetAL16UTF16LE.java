...
class CharacterSetAL16UTF16LE extends CharacterSet implements CharacterRepConstants {
    ...
    public String toStringWithReplacement(byte[] bytes, int offset, int count) {
        try {
            ...
            int chars_len = CharacterSet.convertAL16UTF16LEBytesToJavaChars(bytes, offset, chars,
                                                                            0, count, true);
    ...
    public byte[] convert(CharacterSet from, byte[] source, int offset, int count)
        ...
        if (from.rep == 5) {
            ...
            result = stringToAL16UTF16LEBytes(s);
        }
 ...
 * oracle.sql.CharacterSetAL16UTF16LE JD-Core Version: 0.6.0
 */