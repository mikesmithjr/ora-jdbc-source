...
public class NetInputStream extends InputStream implements SQLnetDef {
    ...
    public int read(byte[] paramArrayOfByte) throws IOException, NetException, BreakNetException {
        return read(paramArrayOfByte, 0, paramArrayOfByte.length);
    }

    public int read(byte[] paramArrayOfByte, int paramInt1, int paramInt2) throws IOException,
            NetException, BreakNetException {
        int i = 0;
        try {
            do {
                if ((this.daPkt == null) || (this.daPkt.availableBytesToRead <= 0)
                        || (this.daPkt.type == 7))
                    getNextPacket();
 ...
 * oracle.net.ns.NetInputStream JD-Core Version: 0.6.0
 */