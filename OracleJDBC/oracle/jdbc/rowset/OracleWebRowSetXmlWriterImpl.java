  ...
  implements OracleWebRowSetXmlWriter
{
    ...
    throws IOException
  {
    ...
    try
    {
      ...
      if (localMap != null)
      {
        for (localIterator = localMap.keySet().iterator(); localIterator.hasNext(); )
        {
          String str = (String)localIterator.next();
          localObject = (Class)localMap.get(str);
          writeElementString("type", str);
          writeElementString("class", ((Class)localObject).getName());
        }
      }
      endElement("map");
      ...
      Object localObject = paramOracleWebRowSet.getSyncProvider();
      writeElementString("sync-provider-name", ((SyncProvider)localObject).getProviderID());
      writeElementString("sync-provider-vendor", ((SyncProvider)localObject).getVendor());
      writeElementString("sync-provider-version", ((SyncProvider)localObject).getVersion());
      writeElementInteger("sync-provider-grade", ((SyncProvider)localObject).getProviderGrade());
      writeElementInteger("data-source-lock", ((SyncProvider)localObject).getDataSourceLock());
  ...
  private void writeBoolean(boolean paramBoolean)
    throws IOException
  {
    this.xmlWriter.write(new Boolean(paramBoolean).toString());
  }
