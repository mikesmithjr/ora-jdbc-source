...
class T2CPreparedStatement extends OraclePreparedStatement
{
  ...
  void prepareBindPreambles(int paramInt1, int paramInt2)
  {
    ...
    int j = this.bindIndicatorSubRange;

    this.bindIndicatorSubRange = i;

    int k = this.bindIndicatorSubRange - j;
    ...
    {
      Binder localBinder = this.lastBinders[m];
      OracleTypeADT localOracleTypeADT;
      int n;
      if (localBinder == this.theReturnParamBinder)
      {
        localOracleTypeADT = (OracleTypeADT)this.returnParamAccessors[m].internalOtype;
        n = 0;
      }
      else
      {
        localOracleTypeADT = arrayOfOracleTypeADT == null ? null : arrayOfOracleTypeADT[m];

        if (this.outBindAccessors == null) {
          n = 0;
        }
        else {
          Accessor localAccessor = this.outBindAccessors[m];
        ...
        if (localBinder == this.theSetCHARBinder) {
          n = (short)(n | 0x4);
        }
      }
      this.bindIndicators[(k++)] = n;

      if (localOracleTypeADT != null)
      {
        long l = localOracleTypeADT.getTdoCState();

        this.bindIndicators[(k + 0)] = (short)(int)(l >> 48 & 0xFFFF);

        this.bindIndicators[(k + 1)] = (short)(int)(l >> 32 & 0xFFFF);

        this.bindIndicators[(k + 2)] = (short)(int)(l >> 16 & 0xFFFF);

        this.bindIndicators[(k + 3)] = (short)(int)(l & 0xFFFF);
      }
